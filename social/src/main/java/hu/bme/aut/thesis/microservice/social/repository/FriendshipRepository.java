...
public interface FriendshipRepository extends JpaRepository<Friendship, Integer> {
    ...
    @Query("select f from Friendship f where f.pending = false and (f.firstUserId = :fromUserId and f.secondUserId = :toUserId or f.firstUserId = :toUserId and f.secondUserId = :fromUserId)")
    Optional<Friendship> findAcceptedByUserId(@Param("fromUserId") Integer fromUserId, @Param("toUserId") Integer toUserId);
